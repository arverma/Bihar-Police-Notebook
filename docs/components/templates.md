...
flowchart TD
  ...
  iframe[Same_page_iframe_print_dialog]
...
- **Print / PDF:** `runPdfExport()` clones the live `.diary-page` / `.letter-page` cards via [`editor/js/print-clone.js`](../../editor/js/print-clone.js), strips screen chrome, and prints them from a hidden same-page iframe with the same editor CSS (unscaled A4). Line wrapping therefore matches what you see while writing.
