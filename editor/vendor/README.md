# Vendored client-PDF libraries

Pinned browser UMD builds used only by the iOS/iPadOS export path
(`editor/js/client-pdf.js`). Desktop native print does not load these files.
...
Loaded lazily on first client-PDF export via classic `<script>` tags (same pattern as Drive GIS).
